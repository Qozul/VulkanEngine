...
const float sampleOffset = 2.0;
const float bokehRadius = 4;
...
vec4 weakGaussian()
{
	...
	for (int x = -4; x <= 4; ++x) {
		for (int y = -4; y <= 4; ++y) {
			...
			result += texture(texSamplers[nonuniformEXT(SC_GEOMETRY_COLOUR_IDX)], inUV + off) * weights[abs(x)] * weights[abs(y)];
		}
	}
	return result;
}
