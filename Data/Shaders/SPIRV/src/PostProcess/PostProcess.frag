...
layout(binding = 0) uniform sampler2D geometryColourTexture;
layout(binding = 1) uniform sampler2D geometryDepthTexture;
...
void main()
{
	...
	if (depth == 1.0) {
		...
		colour = mix(texture(geometryColourTexture, uv), vec4(1.0), depth);
	}
}
