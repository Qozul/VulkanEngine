...
Atmosphere::Atmosphere(AtmosphereParameters params)
	: params_(params)
{
}
...
void Atmosphere::precalculateTextures(const LogicDevice* logicDevice)
{
	...
	initTextures(logicDevice, textures_);

	// Create the uniform buffer for required parameters.
	DescriptorBuffer* buffer = DescriptorBuffer::makeBuffer<UniformBuffer>(logicDevice, MemoryAllocationPattern::kDynamicResource, 0, 0,
		sizeof(AtmosphereParameters), VK_SHADER_STAGE_COMPUTE_BIT);
	...
	AtmosphereParameters* precompData = static_cast<AtmosphereParameters*>(buffer->bindRange());
	*precompData = params_;
	buffer->unbindRange();

	// Create the compute pipelines.
	ComputePipeline transmittancePipeline = ComputePipeline(logicDevice, ComputePipeline::makeLayoutInfo(1, &layout, {}), "AtmosphereAltTransmittance");
	ComputePipeline singleScatteringPipeline = ComputePipeline(logicDevice, ComputePipeline::makeLayoutInfo(1, &layout, {}), "AtmosphereAltScattering");
	ComputePipeline gatheringPipeline = ComputePipeline(logicDevice, ComputePipeline::makeLayoutInfo(1, &layout, {}), "AtmosphereAltGathering");
	ComputePipeline multipleScatteringPipeline = ComputePipeline(logicDevice, ComputePipeline::makeLayoutInfo(1, &layout, {}), "AtmosphereAltMultipleScattering");
	...
	CHECK_VKRESULT(vkQueueSubmit(logicDevice->getQueueHandle(QueueFamilyType::kComputeQueue), 1, &submitInfo, VK_NULL_HANDLE));
	// TODO proper sync rather than just wait idle
	CHECK_VKRESULT(vkQueueWaitIdle(logicDevice->getQueueHandle(QueueFamilyType::kComputeQueue)));
...
void Atmosphere::initTextures(const LogicDevice* logicDevice, PrecomputedTextures& finalTextures)
{
	finalTextures.transmittanceImage = new Image(logicDevice, Image::makeCreateInfo(VK_IMAGE_TYPE_2D, 1, 1, VkFormat::VK_FORMAT_R32G32B32A32_SFLOAT, VkImageTiling::VK_IMAGE_TILING_OPTIMAL,
		VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_LAYOUT_GENERAL, TRANSMITTANCE_TEXTURE_WIDTH, TRANSMITTANCE_TEXTURE_HEIGHT),
		MemoryAllocationPattern::kStaticResource, { VK_IMAGE_VIEW_TYPE_2D });
	...
	finalTextures.gatheringImage = new Image(logicDevice, Image::makeCreateInfo(VK_IMAGE_TYPE_2D, 1, 1, VkFormat::VK_FORMAT_R16G16B16A16_SFLOAT, VkImageTiling::VK_IMAGE_TILING_OPTIMAL,
		VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_LAYOUT_GENERAL, GATHERING_TEXTURE_WIDTH, GATHERING_TEXTURE_HEIGHT),
		MemoryAllocationPattern::kStaticResource, { VK_IMAGE_VIEW_TYPE_2D });
	...
	finalTextures.scatteringImage = new Image(logicDevice, Image::makeCreateInfo(VK_IMAGE_TYPE_3D, 1, 1, VkFormat::VK_FORMAT_R16G16B16A16_SFLOAT, VkImageTiling::VK_IMAGE_TILING_OPTIMAL,
		VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_LAYOUT_GENERAL, SCATTERING_TEXTURE_WIDTH, SCATTERING_TEXTURE_HEIGHT, SCATTERING_TEXTURE_DEPTH),
		MemoryAllocationPattern::kStaticResource, { VK_IMAGE_VIEW_TYPE_3D });
